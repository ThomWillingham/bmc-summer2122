...
*/
void respond(uint8_t r1, int myID, int episode, uint8_t* buffer) {
    if (myID == 0) {
        // We are the legitimate tag
        uint8_t g = hash(r1 ^ r2 ^ k);
...
int main() {
    ...
    if (AI_first_recv == 0) {
        ...
        uint8_t r2_recovered = msg_buffer[0];
        uint8_t match_bits = msg_buffer[1];
        uint8_t g_local = hash(r1r^r2_recovered^k);
        uint8_t ID2_local = ID >> g_local | ID << g_local;
        uint8_t match_local = (ID2_local^g_local) >> (BITS/2);
    ...
    if (AI_second_recv == 0) {
        ...
        printf("Buffer vals after respond call %d, %d\n", msg_buffer[3], msg_buffer[4]);
        // Decide whether we can use r1r to recover that left half of ID2
        uint8_t r2_recovered = msg_buffer[3];
        uint8_t match_bits = msg_buffer[4];
        uint8_t g_local = hash(r1r^r2_recovered^k);
        uint8_t ID2_local = ID >> g_local | ID << g_local;
        uint8_t match_local = (ID2_local^g_local) >> (BITS/2);
        if (match_local == match_bits) {
