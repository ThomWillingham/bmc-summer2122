...
*/
void respond(uint16_t r1, int myID, int episode, uint16_t* buffer) {
    if (myID == 0) {
        // We are the legitimate tag
        uint16_t g = hash(r1 ^ r2 ^ k);
        ...
        */
        ...
        int AI_v2;
        int AI_v3;
        uint16_t v_choices[2 + episode*3];
        v_choices[0] = r1a;
        v_choices[1] = r1;
...
int main() {
    ...
    if (AI_first_recv == 0) {
        ...
        uint16_t r2_recovered = msg_buffer[0];
        uint16_t match_bits = msg_buffer[1];
        uint16_t g_local = hash(r1r^r2_recovered^k);
        uint16_t ID2_local = ID >> g_local | ID << g_local;
        uint16_t match_local = (ID2_local^g_local) >> (BITS/2);
    ...
    if (AI_second_recv == 0) {
        ...
        printf("Buffer vals after respond call %d, %d\n", msg_buffer[3], msg_buffer[4]);
        // Decide whether we can use r1r to recover that left half of ID2
        uint16_t r2_recovered = msg_buffer[3];
        uint16_t match_bits = msg_buffer[4];
        uint16_t g_local = hash(r1r^r2_recovered^k);
        uint16_t ID2_local = ID >> g_local | ID << g_local;
        uint16_t match_local = (ID2_local^g_local) >> (BITS/2);
        if (match_local == match_bits) {
