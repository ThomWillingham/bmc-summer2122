...
uint32_t encrypt(uint32_t in){
    return in;
}

uint32_t decrypt(uint32_t in){
    return in;
}

// cf. https://stackoverflow.com/questions/12700497/how-to-concatenate-two-integers-in-c
uint32_t concatenate(uint16_t x, uint16_t y){
    uint32_t rtn;
    rtn = ( (uint32_t) x << 16) | y;
    return rtn;
}
...
struct big_msg interleave(uint32_t x, uint32_t y){
    struct big_msg rtn = {0, 0};

    // Because of size we'll split into two
    uint16_t x_1 = ((((1UL << 16) - 1) << 16) & x) >> 16;
    uint16_t x_2 = ((1UL << 16) - 1) & x;
    uint16_t y_1 = ((((1UL << 16) - 1) << 16) & y) >> 16;
    uint16_t y_2 = ((1UL << 16) - 1) & y;

    for (int i = 0; i < sizeof(x_1) * 8; i++){
        // Pick out appropriate bits
        uint32_t x_1_m = (x_1 & (1UL << i));
        uint32_t y_1_m = (y_1 & (1UL << i));
    ...
    for (int i = 0; i < sizeof(x_1) * 8; i++){
        // Pick out appropriate bits
        uint32_t x_2_m = (x_2 & (1UL << i));
        uint32_t y_2_m = (y_2 & (1UL << i));
        // Place them appropriately
        rtn.M2 |= (y_2_m << i);
        rtn.M2 |= (x_2_m << (i+1));
    }
...
struct big_msg inv_interleave(uint32_t x, uint32_t y){
    ...
    for (int i = 0; i < sizeof(x) * 8 / 2; i ++){ // x first
        uint32_t x_2_m = (x & (1UL << (2*i)));
        uint32_t x_1_m = (x & (1UL << (2*i+1)));
        x_2 |= (x_2_m >> i);
        x_1 |= (x_1_m >> (i+1));
    }

    for (int i = 0; i < sizeof(y) * 8 / 2; i ++){ // y second
        uint32_t y_2_m = (y & (1UL << (2*i)));
        uint32_t y_1_m = (y & (1UL << (2*i+1)));
        y_2 |= (y_2_m >> i);
        y_1 |= (y_1_m >> (i+1));
    }

    uint32_t comb_1 = concatenate(x_1, y_1);
    uint32_t comb_2 = concatenate(x_2, y_2);
...
int main(){
    uint32_t X_1; // 32 bit msg
    uint32_t X_2; // 32 bit msg
    uint32_t X_3; // 32 bit msg
    uint16_t B; // 16 bit counters
    uint16_t A;
    uint16_t S_base; // has to be free.
    uint16_t S_device; // has to be free
    _Bool goal = 0;
    uint32_t decrypt_M1_1;
    uint32_t decrypt_M2_1;
    uint32_t decrypt_M1_2;
    uint32_t decrypt_M2_2;
    uint32_t decrypt_M1_3;
    uint32_t decrypt_M2_3;
    struct big_msg tmp_msg_1;
    uint32_t extracted_X_1;
    uint16_t extracted_S_1;
    uint16_t extracted_B_1;
    struct big_msg tmp_msg_2;
    uint32_t extracted_X_2;
    uint16_t extracted_S_2;
    uint16_t extracted_B_2;
    struct big_msg tmp_msg_3;
    uint32_t extracted_X_3;
    uint16_t extracted_S_3;
    uint16_t extracted_B_3;
    ...
    // Buffers with 10 entries, each 32 bits that contains the messages sent for M1 and M2.
    uint32_t episodes_M1[10];
    uint32_t episodes_M2[10];

    if (AI_legit_1){ // For base station
        B += 1; // Increment base station counter

        uint32_t tmp_concat = concatenate(S_base, B);
        struct big_msg msg = interleave(X_1, tmp_concat);
        // e.g. X=[1010]; S=[00]; B=[11]; S \concat B = [0011] ;
        //      [M1;M2] = [1000;1101]

        uint32_t enc_M1 = encrypt(msg.M1);
        uint32_t enc_M2 = encrypt(msg.M2);
    ...
    extracted_S_1 = ((((1 << 16) - 1) << 16) & tmp_msg_1.M2) >> 16;
    extracted_B_1 = ((1 << 16) - 1) & tmp_msg_1.M2;
    ...
    extracted_S_2 = ((((1 << 16) - 1) << 16) & tmp_msg_2.M2) >> 16;
    extracted_B_2 = ((1 << 16) - 1) & tmp_msg_2.M2;
    ...
    extracted_S_3 = ((((1 << 16) - 1) << 16) & tmp_msg_3.M2) >> 16;
    extracted_B_3 = ((1 << 16) - 1) & tmp_msg_3.M2;
