/*
 * CHANGES: Make use of structs and arrays as it's much cleaner and
 * I believe CBMC should be able to handle them well. Might need to experiment
 * with this.
 *
 * ISSUE: in the previous set-up (mark2) functions were quite cubersome due to
 * no use of arrays or structs, so to return multiple outputs it was necessary
 * to have an input which determined which output to output.
 */

#include <stdint.h>

__int128 encrypt(__int128 in){
    return in;
}

__int128 decrypt(__int128 in){
    return in;
}

// cf. https://stackoverflow.com/questions/12700497/how-to-concatenate-two-integers-in-c
__int128 concatenate(uint64_t x, uint64_t y){
    __int128 rtn;
    rtn = ( (__int128) x << 64) | y;
    return rtn;
}
...
// cf. https://stackoverflow.com/questions/6400954/what-is-char-bit
struct big_msg interleave(__int128 x, __int128 y){
    struct big_msg rtn = {0, 0};

    // Because of size we'll split into two
    uint64_t x_1 = ((((1UL << 64) - 1) << 64) & x) >> 64;
    uint64_t x_2 = ((1UL << 64) - 1) & x;
    uint64_t y_1 = ((((1UL << 64) - 1) << 64) & y) >> 64;
    uint64_t y_2 = ((1UL << 64) - 1) & y;

    for (int i = 0; i < sizeof(x_1) * 8; i++){
        // Pick out appropriate bits
        __int128 x_1_m = (x_1 & (1UL << i));
        __int128 y_1_m = (y_1 & (1UL << i));
    ...
    for (int i = 0; i < sizeof(x_1) * 8; i++){
        // Pick out appropriate bits
        __int128 x_2_m = (x_2 & (1UL << i));
        __int128 y_2_m = (y_2 & (1UL << i));
        // Place them appropriately
        rtn.M2 |= (y_2_m << i);
        rtn.M2 |= (x_2_m << (i+1));
    }
...
struct big_msg inv_interleave(__int128 x, __int128 y){
    ...
    for (int i = 0; i < sizeof(x) * 8 / 2; i ++){ // x first
        __int128 x_2_m = (x & (1UL << (2*i)));
        __int128 x_1_m = (x & (1UL << (2*i+1)));
        x_2 |= (x_2_m >> i);
        x_1 |= (x_1_m >> (i+1));
    }

    for (int i = 0; i < sizeof(y) * 8 / 2; i ++){ // y second
        __int128 y_2_m = (y & (1UL << (2*i)));
        __int128 y_1_m = (y & (1UL << (2*i+1)));
        y_2 |= (y_2_m >> i);
        y_1 |= (y_1_m >> (i+1));
    }

    __int128 comb_1 = concatenate(x_1, y_1);
    __int128 comb_2 = concatenate(x_2, y_2);
...
int main(){
    __int128 X_1; // 128 bit msg
    __int128 X_2; // 128 bit msg
    __int128 X_3; // 128 bit msg
    uint64_t B; // 64 bit counters
    uint64_t A;
    uint64_t S_base; // has to be free.
    uint64_t S_device; // has to be free
    _Bool goal = 0;
    __int128 decrypt_M1_1;
    __int128 decrypt_M2_1;
    __int128 decrypt_M1_2;
    __int128 decrypt_M2_2;
    __int128 decrypt_M1_3;
    __int128 decrypt_M2_3;
    struct big_msg tmp_msg_1;
    __int128 extracted_X_1;
    uint64_t extracted_S_1;
    uint64_t extracted_B_1;
    struct big_msg tmp_msg_2;
    __int128 extracted_X_2;
    uint64_t extracted_S_2;
    uint64_t extracted_B_2;
    struct big_msg tmp_msg_3;
    __int128 extracted_X_3;
    uint64_t extracted_S_3;
    uint64_t extracted_B_3;
    ...
    // Buffers with 10 entries, each 128 bits that contains the messages sent for M1 and M2.
    __int128 episodes_M1[10];
    __int128 episodes_M2[10];

    if (AI_legit_1){ // For base station
        B += 1; // Increment base station counter

        __int128 tmp_concat = concatenate(S_base, B);
        struct big_msg msg = interleave(X_1, tmp_concat);
        // e.g. X=[1010]; S=[00]; B=[11]; S \concat B = [0011] ;
        //      [M1;M2] = [1000;1101]

        __int128 enc_M1 = encrypt(msg.M1);
        __int128 enc_M2 = encrypt(msg.M2);
    ...
    extracted_S_1 = ((((1 << 64) - 1) << 64) & tmp_msg_1.M2) >> 64;
    extracted_B_1 = ((1 << 64) - 1) & tmp_msg_1.M2;
    ...
    extracted_S_2 = ((((1 << 64) - 1) << 64) & tmp_msg_2.M2) >> 64;
    extracted_B_2 = ((1 << 64) - 1) & tmp_msg_2.M2;
    ...
    extracted_S_3 = ((((1 << 64) - 1) << 64) & tmp_msg_3.M2) >> 64;
    extracted_B_3 = ((1 << 64) - 1) & tmp_msg_3.M2;
